...
class Cart {
	...
	public function add($product_id, $seller_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND seller_id = '" . (int)$seller_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");

		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', seller_id = '" . (int)$seller_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
		} else {
...
public function allclear() {
  ...
  public function getSellerproducts($seller_id) {
		...
		$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'AND seller_id = '" . $seller_id . "'");
		...
		foreach ($cart_query->rows as $cart) {
          ...
          'seller_id'       => $product_query->row['seller_id'],
				...
				$this->sellerremove($cart['cart_id'], $seller_id);
			}
		}
  	...
  	public function getSellerrecurringProducts($seller_id) {
		$product_data = array();

		foreach ($this->getSellerproducts($seller_id) as $value) {
			if ($value['recurring']) {
				$product_data[] = $value;
			}
		}
	...
	public function getSellerweight($seller_id) {
		$weight = 0;
	...
	public function getSellertaxes($seller_id) {
		$tax_data = array();
	...
	public function hasSellerproducts($seller_id) {
		return count($this->getSellerproducts($seller_id));
	}

	public function hasSellerrecurringProducts($seller_id) {
		return count($this->getRecurringProducts($seller_id));
	}

	public function hasSellerstock($seller_id) {
		foreach ($this->getSellerproducts($seller_id) as $product) {
			if (!$product['stock']) {
				return false;
			}
	...
	public function hasSellershipping($seller_id) {
		foreach ($this->getSellerproducts($seller_id) as $product) {
			if ($product['shipping']) {
				return true;
			}
	...
	public function hasSellerdownload($seller_id) {
		foreach ($this->getSellerproducts($seller_id) as $product) {
			if ($product['download']) {
				return true;
			}
